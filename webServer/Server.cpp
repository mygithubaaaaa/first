...
int Server::ServerStartup() {
	...
	ifstream readconfig("config3.txt");
	if (!readconfig)
	{
	...
	char *temp2 = new char[configlength];
	char *temp3 = new char[configlength];

	readconfig.getline(temp, configlength,0);
	string s = string(temp);
	readconfig.seekg(0, std::ios::beg);
	readconfig.getline(temp2, configlength);
	readconfig.getline(temp2, configlength);
	readconfig.getline(temp3, configlength);

	this->maincontents = string(temp3);
	//cout << s;
	//cout << this->maincontents << endl;
	readconfig.close();
	...
	cout << "读取的配置:\n" << "ip:" << Serverip << "\nport:" << Serverport << "\nmaincontents:"<<this->maincontents<<endl;
...
int Server::response2(SOCKET sock) {
	...
	while (rtn !=0 && rtn != SOCKET_ERROR) {
		...
		//}
		string request = url;
		url = this->maincontents + url;
		FILE *infile = fopen(url.c_str(), "rb");
		if (infile == NULL) {
			//output << typeStr << std::endl;
			output << "file name:" << request << " file open error,may no this file ,please check the name .发送404" << std::endl;

			//std::cout << output.str();
			//fclose(infile);
			//return 0;
			request = "404.html";
			url = this->maincontents+"/404.html";
